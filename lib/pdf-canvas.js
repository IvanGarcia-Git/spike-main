...
export function buildPdfCanvasOptions({ background = '#ffffff', scale = 2.5, fontFamily = 'Arial, sans-serif' } = {}) {
  return {
    scale,
    useCORS: true,
    ...
    onclone: (clonedDoc) => {
      ...
      allElements.forEach((el) => {
        ...
        if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
          ...
          div.style.lineHeight = 'normal'; // 'normal' coincide con la métrica baseline de html2canvas (mide a line-height normal) → texto centrado por el flex del contenedor
          div.style.padding = '0';
