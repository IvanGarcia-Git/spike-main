...
export default function LeadsCallsComponent({
  leads,
  selectedDate,
  onPreviousDay,
  onNextDay,
  ...
  const completedLeads = leads.filter((item) => item.completed).length;
  ...
  return (
    <div className="flex flex-col mt-4">
      <div className="p-4 bg-white rounded-lg shadow-lg w-full max-w-md mx-auto mb-2">
        ...
        <ul className="space-y-2">
          {leads.length > 0 ? (
            ...
            <p className="text-gray-500 text-sm">
              No hay llamadas para esta fecha.
            </p>
          )}
        </ul>
      </div>
