...
export default function ChangeChannelModal({
  setIsChannelChangeModalOpen,
  selectedChannelForBatch,
  setSelectedChannelForBatch,
  channels,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4 lg:ml-72 text-black">
      <div className="bg-white p-6 md:p-8 rounded-lg shadow-xl w-full max-w-md">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Cambiar canal</h2>
          <button
            onClick={() => {
        ...
        <div className="mb-6">
          ...
          <select
            value={selectedChannelForBatch?.id || ""}
            onChange={(e) => {
              const selectedChannel = channels.find(
                (channel) => channel.id === parseInt(e.target.value)
              );
              setSelectedChannelForBatch(selectedChannel);
            }}
        ...
        <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3">
          ...
          <button
            onClick={handleBatchChannelChange}
