...
export default function ContractNewEventModal({
  isModalOpen,
  setIsModalOpen,
  contract,
  customer
  ...
  const handleAddTask = async (e) => {
    ...
    const contractUrl = `/contratos/${customer.uuid}/${contract.uuid}`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          `Error agregando la tarea: ${errorData.message || "Error desconocido"
          }`
        );
      }
    } catch (error) {
  ...
  const handleAddReminder = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        alert(
          `Error agregando el recordatorio: ${errorData.message || "Error desconocido"
          }`
        );
      }
    } catch (error) {
  ...
  const handleAddLeadCall = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/lead-calls/`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${jwtToken}`,
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        alert(
          `Error agregando la llamada: ${errorData.message || "Error desconocido"
          }`
        );
      }
    } catch (error) {
  ...
  return (
    <div
      className={`bg-foreground text-black p-6 rounded-lg shadow-lg w-full max-w-lg ${isModalOpen ? "" : "hidden"
        }`}
    >
      <h3 className="text-xl font-bold mb-4 text-black">Crear nueva tarea</h3>
      <div className="flex mb-4 border-b">
        <button
          className={`px-4 py-2 ${activeTab === "task"
        ...
        <button
          className={`px-4 py-2 ${activeTab === "reminder"
              ? "border-b-2 border-blue-500 font-bold"
              : "text-gray-500"
            }`}
        ...
        <button
          className={`px-4 py-2 ${activeTab === "leadCall"
              ? "border-b-2 border-blue-500 font-bold"
              : "text-gray-500"
            }`}
        ...
        <form onSubmit={handleAddReminder}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              onChange={(e) =>
                setNewReminder({ ...newReminder, subject: e.target.value })
              }
              required
            />
          </div>
        ...
        <form onSubmit={handleAddLeadCall}>
          <div className="mb-4">
            ...
            <input
              ...
              onChange={(e) =>
                setNewLeadCall({ ...newLeadCall, startDate: e.target.value })
              }
              required
            />
          </div>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              onChange={(e) =>
                setNewLeadCall({ ...newLeadCall, subject: e.target.value })
              }
              required
            />
          </div>
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              onChange={(e) =>
                setNewLeadCall({ ...newLeadCall, observations: e.target.value })
              }
            ></textarea>
          </div>
